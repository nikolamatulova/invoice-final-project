...
const PersonForm = () => {
    ...
    // React Router hook to programmatically navigate after the submission
    const navigate = useNavigate();
    
    // Extract 'id' param from URL to know if we are editing an existing person
    const {id} = useParams();
    
    // State for the person form data, with all initial fields set to empty or default values
    ...
    // States to track the submission result and possible errors
    const [sentState, setSent] = useState(false);
    const [successState, setSuccess] = useState(false);
    const [errorState, setError] = useState(null);

    // On component mount or when `id` changes, fetch missing person data if editing
    useEffect(() => {
        if (id) {
            // Fetch person's data by ID and update the state to fill the form for editing
    ...
    const handleSubmit = (e) => {
        e.preventDefault(); // Prevent default form submit behaviour (page reload)

        // Use PUT if editing (id present), otherwise POST to create new person
        (id 
        ? apiPut("/api/persons/" + id, person) 
        : apiPost("/api/persons", person))
            .then((data) => {
                // If successful, mark sent and success, the navigate back to persons list
                setSent(true);
                setSuccess(true);
                navigate("/persons");
            })
            .catch((error) => {
                // On error, save error message to display and mark sent with failure
    ...
    // Local copies of sent and successs state for easier usage in JSX
