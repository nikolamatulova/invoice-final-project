...
public interface PersonRepository extends JpaRepository<PersonEntity, Long>, JpaSpecificationExecutor<PersonEntity> {
    ...
    /**
     * Gets statistics for all people, including their total sales revenue
     * @return a list of statistics arrays for each person
     */
    @Query("SELECT p.id, p.name, COALESCE(SUM(i.price), 0) " +
